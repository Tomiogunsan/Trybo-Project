...
export default function ProfileSideBar({listings}) {
 ...
 <div className=' shadow-md fixed '>
    {sideBar.map((item, idx) => {
          ...
          {listings.length === 0 ?  <div  key={idx} className={ `flex  p-8  ${sideBar.length - 1 === idx ? " border-b-0 border": 'border-b-[1px] border'}`}>
           { item.link !== "listings"  && <Link to={item.link } smooth={true}  
            duration={1000} activeClass='active' className='flex'>
            {item.label}
             <div className='ml-4 text-lg text-[#C0C0C0]'>{ item.text}</div>
             </Link>} : <Link to={item.link } smooth={true}  
            duration={1000} activeClass='active' className='flex'>
            {item.label}
             <div className='ml-4 text-lg text-[#C0C0C0]'>{ item.text}</div>
             </Link>
