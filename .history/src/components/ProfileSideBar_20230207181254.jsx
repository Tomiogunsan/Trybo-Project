...
export default function ProfileSideBar() {
    ...
    const sideBar = [
        {
            label: <BsFillPersonFill className='bg-[#f5f5f5] rounded-full text-[#1a1e24] text-4xl'/>,
           text: 'Profile',
        },
        {
            label: <BsFillPersonCheckFill className='bg-[#f5f5f5] rounded-full text-[#1a1e24] text-4xl'/>,
           text: 'Verification',
        },
        {
            label: <FiSettings className='bg-[#f5f5f5]  rounded-full text-[#C0C0C0] text-4xl'/>,
           text: 'Settings',
        },
    ]
  return (
 <div>
    {sideBar.map((item, idx) => (
        <div  key={idx} className={ `flex  p-8  ${sideBar.length - 1 === idx ? " border-b-0 border": 'border-b-2 border'}`}>
           {item.label}
            <div className='ml-4 text-lg text-[#1a1e24]'>{item.text}</div>
        </div>
