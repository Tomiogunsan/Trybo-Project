...
export default function ProfileSideBar() {
 ...
 <div>
    {sideBar.map((item, idx) => (
        <div key={idx} className={ `${sideBar.length - 1 === idx ? " border-b-0 border": 'border-b-2 border'}`}>
           <div className='bg-[#556987] rounded-full text-[#1a1e24]text-4xl'>{item.label}</div> 
            <div>{item.text}</div>
        </div>
