...
export default function ProfileSideBar() {
 ...
 <div className='shadow-md'>
    {sideBar.map((item, idx) => (
        <div  key={idx} className={ `flex  p-8 fixed  ${sideBar.length - 1 === idx ? " border-b-0 border": 'border-b-[1px] border'}`}>
           <Link to={item.link} smooth={true}  duration={500} className='flex'>
           {item.label}
